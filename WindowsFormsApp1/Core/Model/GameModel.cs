    ...
    public static class GameModel
    {
        ...
        private static void InitGameObjects()
        {
            ...
            for (int i = 0; i < EnemiesCount; i++)
                Enemies.Add(new EnemyCar(Paths.PathToRedCarImage, 
                    Traffic_Racer.GameField.Width / EnemiesCount * i, new Random().Next(-1000, 0)));
        }
        ...
        public static void СheckCollision()
        {
            if (Traffic_Racer.canMove)
                foreach (var enemy in Enemies)
                    if (Car.Width + enemy.Width-15 >= Math.Max(Car.Width + Car.X, enemy.Width + enemy.X)
                    - Math.Min(Car.X, enemy.X) &&
                    Car.Height + enemy.Height-15 >= Math.Max(Car.Height + Car.Y, enemy.Height + enemy.Y)
                    - Math.Min(Car.Y, enemy.Y))
                        Traffic_Racer.canMove = false;
            //Traffic_Racer.Controls.Add(resetButton);
